...
The free workflow
Convert your .stl / .3mf / .obj to .rsdocx with this tool (leave Smooth seams off).
Open the .rsdocx in free DesignSpark Mechanical Explorer.
In the structure tree, right-click the mesh body → Convert to Solid → Merge faces.
You now have an editable solid — push/pull faces, cut holes, combine, save.
...
Download & run
Download stl2rsdocx_gui.exe from the Releases page.
Double-click it. No install, no dependencies.
(First run) Windows SmartScreen may warn about an unknown publisher — click More info -> Run anyway. Normal for unsigned tools.
Supported files
Drop in...
You get...
Then in DSM
STL (.stl)
mesh .rsdocx
open → Convert to Solid → editable solid
3MF (.3mf)
mesh .rsdocx
open → Convert to Solid → editable solid
OBJ (.obj)
...
open → Convert to Solid → editable solid
STEP (.stp, .step)
exact face curves .txt + mesh .rsdocx
Fill / Revolve the curves, or Convert to Solid the mesh
DXF (.dxf)
2D profile .txt
open → Fill → Pull to thickness
SVG (.svg)
2D profile .txt
open → Fill → Pull to thickness
...
How to use it
Add files - drag onto the window, drop on the .exe icon, or Add files... / Add folder....
Set options (every control has a hover tooltip):
...
Convert - the output appears next to your file.
...
Best for
STEP solid (.step)
a faceted solid body
machine shops, SolidWorks/Fusion/FreeCAD — opens as a real solid
3D mesh DXF (.dxf)
loose 3D triangles
AutoCAD; just need the shape
2D outline DXF (.dxf)
silhouette + holes (pick a plane)
laser / CNC / waterjet flat profiles
...
In DSM: File → Save As → STL (free in Explorer). Choose the finest tessellation you can — this controls how round your curves stay.
In this tool's Export tab: drop that STL in, pick STEP solid, Export.
Send the .step to your shop. It opens in any CAD as a solid.
...
Not affiliated with, endorsed by, or supported by RS Group / DesignSpark or Ansys. "DesignSpark Mechanical" and "SpaceClaim" are trademarks of their respective owners. Use at your own risk.
